...
def get_scaler_from_data_list(data_list, key, scaler_type='standard'):
    """
    基于数据列表创建标准化器
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method, prop_list=None):
    """预处理数据集，确保必需的属性被包含
    
    Args:
        input_file: 数据集文件路径
        num_workers: 预处理工作线程数
        niggli: 是否应用 niggli 约简
        primitive: 是否转换为原胞
        graph_method: 图构建方法
        prop_list: 要包含的属性列表
    ...
    # 确保必需的属性在列表中
    required_props = ['formation_energy_per_atom']
    ...
    def process_one(row, niggli, primitive, graph_method, prop_list):
        ...
        for k in prop_list:
            ...
            if k in row:
                ...
                print(f"警告: 属性 '{k}' 在数据中未找到")
                print(f"可用的列: {list(row.index)}")
                # 对于必需属性，设置默认值
                if k in required_props:
                    properties[k] = 0.0
    ...
    if len(ordered_results) > 0:
        print(f"处理后的结果包含的键: {list(ordered_results[0].keys())}")
        for prop in prop_list:
            if prop not in ordered_results[0]:
                print(f"警告: 处理后的结果中缺少属性 '{prop}'")

    return ordered_results


def preprocess_tensors(crystal_array_list, niggli, primitive, graph_method, prop_list=None):
    """预处理张量数据，确保必需的属性被包含
    
    Args:
        crystal_array_list: 晶体数组列表
        niggli: 是否应用 niggli 约简
        primitive: 是否转换为原胞
        graph_method: 图构建方法
        prop_list: 要包含的属性列表（可选）
    ...
    def process_one(batch_idx, crystal_array, niggli, primitive, graph_method):
        ...
        # 如果晶体数组中包含属性信息，添加到结果中
        # 打印晶体数组中可用的键
        if batch_idx == 0:  # 只打印第一个晶体数组的键，避免过多输出
            print(f"晶体数组中可用的键: {list(crystal_array.keys())}")
            
        for prop in prop_list:
            if prop in crystal_array:
                result_dict[prop] = crystal_array[prop]
            else:
                # 对于必需属性，设置默认值
                if prop in required_props:
                    print(f"警告: 必需属性 '{prop}' 在晶体数据中未找到，使用默认值")
                    result_dict[prop] = 0.0
                else:
                    print(f"警告: 属性 '{prop}' 在晶体数据中未找到")
